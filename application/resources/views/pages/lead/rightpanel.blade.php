    ...
    <div class="x-section">
        ...
        <div class="x-element text-center font-14" id="card-lead-element-container-name">
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" id="card-lead-name"
                    data-container=".card-modal" tabindex="0" data-popover-content="card-lead-name-popover"
        ...
        <div class="x-element" id="card-lead-category"><i class="mdi mdi-folder"></i>
            ...
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal"
                    id="card-lead-category-text" tabindex="0" data-popover-content="card-lead-category-popover"
        ...
        <div class="x-element"><i class="mdi mdi-phone"></i> <span>{{ cleanLang(__('lang.telephone')) }}: </span>
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal"
                    id="card-lead-phone" tabindex="0" data-popover-content="card-lead-phone-popover"
                    data-value="{{ $lead->lead_phone }}"
        ...
        <!--password-->
        <div class="x-element"><i class="mdi mdi-phone"></i> <span>Password: </span>
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal"
                    id="card-lead-password" tabindex="0" data-popover-content="card-lead-password-popover"
                    data-value="{{ $lead->shown_password }}"
                    data-title="password">{{ $lead->shown_password ?? '---' }}</span>
            @else
                <span class="x-highlight">{{ $lead->shown_password  ?? '---' }}</span>
            @endif
        </div>
        ...
        <div class="x-element"><i class="mdi mdi-email"></i> <span>{{ cleanLang(__('lang.email')) }}: </span>
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal"
                    id="card-lead-email" tabindex="0" data-popover-content="card-lead-email-popover"
        ...
        <div class="x-element" id="card-lead-source"><i class="mdi mdi-magnify-plus"></i>
            ...
            @if ($lead->permission_edit_lead)
                <span class="x-highlight x-editable js-card-settings-button-static" data-container=".card-modal"
                    id="card-lead-source-text" tabindex="0" data-popover-content="card-lead-source-popover"
    ...
    <div class="x-section">
        ...
        @if ($lead->permission_edit_lead && $lead->lead_converted == 'no')
            <div class="x-element x-action js-lead-convert-to-customer" id="card-lead-milestone" tabindex="0"
                ...
                data-popover-content="card-lead-milestones"
                data-title="{{ cleanLang(__('lang.convert_to_customer')) }}"><i class="mdi mdi-redo-variant"></i>
        ...
        @if ($lead->permission_edit_lead && runtimeArchivingOptions())
            <div class="x-element x-action confirm-action-info  {{ runtimeActivateOrAchive('archive-button', $lead->lead_active_state) }} card_archive_button_{{ $lead->lead_id }}"
                ...
                data-url="{{ url('/') }}/leads/{{ $lead->lead_id }}/archive"><i class="mdi mdi-archive"></i>
                <span class="x-highlight" id="lead-start-date">{{ cleanLang(__('lang.archive')) }}</span></span>
            </div>
        @endif
        ...
        @if ($lead->permission_edit_lead && runtimeArchivingOptions())
            <div class="x-element x-action confirm-action-info  {{ runtimeActivateOrAchive('activate-button', $lead->lead_active_state) }} card_restore_button_{{ $lead->lead_id }}"
                ...
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="PUT"
                data-url="{{ url('/') }}/leads/{{ $lead->lead_id }}/activate"><i class="mdi mdi-archive"></i>
                <span class="x-highlight" id="lead-start-date">{{ cleanLang(__('lang.restore')) }}</span></span>
            </div>
        @endif
    ...
    <div class="hidden" id="card-lead-phone-popover">
        ...
        <!--lead - password -->
        <div class="hidden" id="card-lead-password-popover">
            <div class="form-group row m-b-10">
                <div class="col-sm-12 ">
                    <input type="text" class="form-control form-control-sm" id="lead_password" name="lead_password"
                    placeholder="">
                </div>
            </div>
            <div class="form-group text-right">
                <button type="button" class="btn btn-danger btn-sm" id="card-leads-update-password-button"
                    data-progress-bar='hidden' data-url="{{ url('/leads/' . $lead->lead_id . '/update-password') }}"
                    data-type="form" data-ajax-type="post" data-form-id="popover-body">
                    {{ cleanLang(__('lang.update')) }}
                </button>
            </div>
        ...
        <div class="form-group m-t-10">
            @if (config('system.settings_leads_allow_new_sources') == 'yes')
                <input type="text" name="lead_source" id="lead_source"
                    class="col-12 form-control form-control-sm" list="sources">
                <datalist id="sources">
            ...
            @else
                <select class="custom-select col-12 form-control form-control-sm" id="lead_source"
                    name="lead_source">
                    @foreach ($sources as $source)
                        <option value="{{ $source->leadsources_title }}">
